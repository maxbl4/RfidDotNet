...
namespace maxbl4.RfidDotNet.GenericSerial.Tests
{
    ...
    public class SerialPortTests
    {
        ...
        {
            using var port = new SerialPort(serial.ConnectionString.Serial.Port, serial.ConnectionString.Serial.BaudRate, Parity.None, 8, StopBits.One);
            port.Open();
        ...
        public void Should_get_serial_raw_multiple_times()
        {
            ...
            for (var i = 0; i < 10; i++)
            {
                ...
                var read = port.Read(b, 0, 100);
                read.Should().Be(1);
                b[0].Should().Be(expectedResponse[0]);
            }
        }
    }
}